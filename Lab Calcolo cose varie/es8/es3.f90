module gravitazione
implicit none
integer,parameter :: kr=selected_real_kind(15)
real(kind=kr),parameter :: G=6.673E-11_kr  ! costante di gravitazione universale in unita' SI

contains
  subroutine interazione(mass, pos,f,epot, nbody)
...
! in cui veniva calcolata la forza e l'energia potenziale
   ...
   real(kind=kr), intent(in), dimension(:) :: mass
...
use gravitazione
    implicit none
     ...
     real(kind=kr), dimension(:), allocatable::mass
     real(kind=kr), dimension(:,:), allocatable:: pos,vel,f, vel_i, pos_i !prima componente x seconda y
     integer :: nstep,it, nbody
     ...
     allocate(mass(nbody))
     allocate(pos(3,nbody), vel(3, nbody), f(3, nbody))    
    ...
    vel_i = vel
     ...
     call interazione(mass, pos,f,epot,nbody)
     ekin =  sum(0.5 * spread(mass,1,3) * vel**2)/2
     write(unit=2,fmt=*)it,dt*it,ekin,epot,ekin+epot
     
     do it = 1,nstep
        pos = pos + vel * dt + 0.5* f/spread(mass,1,3) * dt**2
        vel = vel + 0.5 * dt * f/spread(mass,1,3)         !  prima parte della formula per le velocita'
        call interazione(mass, pos,f,epot, nbody)
        vel = vel + 0.5 * dt * f/spread(mass,1,3)         !  la formula per le velocita' viene completata qui
        write(unit=1,fmt=*)it,it*dt,pos,vel
        ekin = sum(0.5 * spread(mass,1,3) * vel**2)
